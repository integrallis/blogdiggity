class Blogdiggity::ApplicationController < ApplicationController
  ...
  helper_method :current_contributor, :active_contributor, :require_active_contributor, :admin?
  ...
  def admin?
    current_contributor ? current_contributor.status == 'admin' : false
  end
  ...
  def require_admin_contributor
    redirect_to(root_path, {:alert => "<strong>Hey!</strong> You are not authorized to do that!".html_safe}) if !admin?
  end
end
